...
ros::Publisher frenet_lqr_;           // 发布cmd_vel
ros::Publisher path_pub_;             // 发布参考路径
ros::Publisher trajectory_path;       // 发布实际轨迹

nav_msgs::Path path;                  // 发布参考路径   
geometry_msgs::PoseStamped pose;
nav_msgs::Path trajectorypath;        // 发布实际轨迹
geometry_msgs::PoseStamped this_pose_stamped;
geometry_msgs::Twist vel_msg;         // 发布cmd_vel  
/********************************五次多项式路径规划change_line******************************************/


// double T = 50;                        // t-t0经历的时间
...
// // 终点状态 d
// std::array<double, 3> y_start{50.0, 0.0, 0.0};
...
double vx = 0.01;                                 // 纵向速度 定义为全局变量    
double vy = 0;                                    // 横向速度
double cf = -65494.663, cr = -115494.663;         // 轮胎侧偏刚度 质心侧偏角视为不变  ****

// 前后悬架载荷
double mass_fl = 500, mass_fr = 500, mass_rl = 520, mass_rr = 520;
double mass_front = mass_fl + mass_fr;
double mass_rear = mass_rl + mass_rr;
double m = mass_front + mass_rear;                // 整车质量997kg


double max_lateral_acceleration = 5.0;            // 最大纵向加速度
double standstill_acceleration = -3.0;            // 最大制动减速度
double wheel_base = 2.49;                         // 轴距
double a = wheel_base * (1.0 - mass_front / m);   // 前轴中心到质心的距离
double b = wheel_base * (1.0 - mass_rear / m);    // 后轴中心到质心的距离
double Iz = std::pow(a, 2) * mass_front + std::pow(b, 2) * mass_rear;
                                                  // 车辆绕z轴转动的转动惯量
double wheel_max_degree = 0.54;                   // 轮胎最大转角(rad) 30.9度
 ...
 */
std::array<double, 3> calQuaternionToEuler(const double x, const double y, const double z, const double w) 
{
...
void spline_path()
{
  cpprobotics::Vec_f wx({0.0, 9.76, 80.04, 104.02, 125.33, 136.96, 149.52, 162.99, 171.18, 180.28, 188.13, 192.47});
  cpprobotics::Vec_f wy({0.0, -2.86, -24.85, -32.01, -24.73, 12.07, 53.66, 94.00, 126.30, 171.32, 215.51, 251.60});
  ...
  {
      cpprobotics::Poi_f point_pose = csp_obj.calc_postion(i);
      float point_yaw = csp_obj.calc_yaw(i);
      float point_curvature = csp_obj.calc_curvature(i);
      fp.x.push_back(point_pose[0]);
      fp.y.push_back(point_pose[1]);
      fp.threat.push_back(point_yaw);
      fp.k.push_back(point_curvature);         // 压入曲率
      
      pose.pose.position.x = point_pose[0];
      pose.pose.position.y = point_pose[1];
...
//   QuinticPolynomial lon_qp(x_start[0], x_start[1], x_start[2], x_end[0], x_end[1], x_end[2], T);        // 纵向 -> 多项式系数
//   QuinticPolynomial lat_qp(y_start[0], y_start[1], y_start[2], y_end[0], y_end[1], y_end[2], T, xend);  // 横向 -> 多项式系数  
...
//     fp.k.push_back(lat_qp.calc_point_k(y_x_dd, y_x_d));         // 压入曲率
...
//     fp.threat.push_back(lat_qp.calc_point_thetar(dy, dx));      // 压入航向角
//   }
//   // 最后一个道路航向角和前一个相同
                                ...
                                std::array<double, 3> calRPY) 
{
  ...
  double phi = calRPY[2];
  double ed = nor.transpose() * d_err;                                      // nor.transpose()对nor转置
  // double ed = -vx*sin();
  double es = tor.transpose() * d_err;
  double projection_point_threat = fp.threat[index] + fp.k[index] * es;     // 投影点的threat角度
  // double phi = fp.threat[index];
  double ed_d = vy * cos(phi - projection_point_threat) + vx * sin(phi - projection_point_threat);
  // double ephi = sin(phi - projection_point_threat);
  double ephi = phi - projection_point_threat;                              // 计算ephi  
  ...
  double projection_point_curvature = fp.k[index];                          // 计算投影点曲率k
                                     ...
                                     Eigen::Matrix<double, 1, 1> R) 
{
  ...
  int numLoop = 200;                      // 设置最大循环迭代次数
  double minValue = 10e-10;               // 设置目标极小值
...
{
  // 计算frenet坐标系下横向航向误差及其变化率，索引点曲率
  std::array<double, 5> err_k = cal_err_k(currentPositionX, currentPositionY, cal_RPY);
  // u = -k * X_e , 计算负反馈矩阵系数
  Eigen::Matrix<double, 1, 4> k = cal_k(err_k);
  // 计算前馈控制量 delta_f
  double forword_angle = cal_forword_angle(k, err_k);
  // u = -k * X_e + delta_f ; Q = [60, 1, 1, 1] R=[35]
  double tempangle = cal_angle(k, forword_angle, err_k);
  // 前轮转角机械限幅 35度 0.6rad
  double angle = limitSterringAngle(tempangle, -wheel_max_degree, wheel_max_degree);
...
int main(int argc, char **argv) {
  ...
  ros::init(argc, argv, "lqr");                                                       // 创建节点
  ros::NodeHandle nh;                                                                 // 创建节点句柄
  frenet_lqr_ = nh.advertise<geometry_msgs::Twist>("/smart/cmd_vel", 20, true);       // 创建Publisher，发送经过lqr计算后的转角及速度
  path_pub_ = nh.advertise<nav_msgs::Path>("rviz_path", 20, true);                    // 创建Publisher，发布规划轨迹
  trajectory_path = nh.advertise<nav_msgs::Path>("trajector_ypath", 20, true);        // 创建Publisher，发布小车运动轨迹
  ros::Subscriber carVel = nh.subscribe("/smart/velocity", 20, velocityCall);         // 订阅车辆速度信息
  ros::Subscriber carPose = nh.subscribe("/smart/rear_pose", 20, poseCallback);       // 订阅车辆后轮中点位置
  ...
  // calc_frenet_paths();                                                                //初始化五次多项式轨迹 -> fp 横纵状态

  // // 打印五次多项式路径序列点
  // int Num = fp.x.size();
  // for (int i = 0; i < Num; i++) {
  //   printf("x,y,th,k,i=%.3f,%.3f,%.3f,%f,%d \n", fp.x[i], fp.y[i], fp.threat[i], fp.k[i], i);
  // }
